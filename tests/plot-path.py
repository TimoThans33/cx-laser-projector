...
def Arrowheadfun(Coords, size=-10, width=0.01):
    size=-int(size)
    index = int(size/20)
    new_coordx=Coords[0]-Coords[0][0]
    new_coordy=Coords[1]-Coords[1][0]
    #Create Arrowhead
    diffx=new_coordx[size]-new_coordx[-1]
    diffy=new_coordy[size]-new_coordy[-1]
    PerpendicularRC = -diffx/diffy
    B = new_coordy[size]-PerpendicularRC*new_coordx[size]
    #WidthArrow = 2*abs(Y-PerpendicularRC*X-B)/sqrt(PerpendicularRC**2+1)


    Y0 = new_coordy[-1]
    Y1 = new_coordy[size]-width
    Y2 = new_coordy[size]+width
    Y3 = new_coordy[-1]
    ...
    Arrowhead = np.array([Xarrow, Yarrow])
    return Arrowhead
...
for i in range(len(x_robot)):
    ...
    #    plt.show()
    plt.plot(np.transpose(rot_coord)[0]-np.transpose(rot_coord)[0][0], np.transpose(rot_coord)[1]-np.transpose(rot_coord)[1][0], color='C0')
    Arrowhead=Arrowheadfun(np.transpose(rot_coord), t/4, width=0.015)
    plt.plot(Arrowhead[0], Arrowhead[1], color='C0')
